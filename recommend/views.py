...
def contact(request):
    if request.method == 'POST':
        ...
        url = 'https://ozlt5p7oqd.execute-api.us-east-1.amazonaws.com/v1/contactme'
        headers = {'Content-Type': 'application/json'}
        data = {
            'req_name': name,
            'req_email': email,
            'req_contact': contacts,
            'req_subject': subject,
            'req_message': message,
        }
        r = requests.post(url, json=data, headers=headers)
        if r.status_code == 200:
            messages.success(request, 'Message sent successfully. We will contact you asap...')
            return redirect('index')
        else:
            messages.warning(request, 'Error while sending message...')
